...
void ofxCvBlobTracker::trackBlobs( const vector<ofxCvBlob>& _blobs ) {
	...
	for( i=0; i<cursize; i++ ){
		...
		for( j=0; j<prevsize; j++ ) {
            ...
            // not working
            /*
            float deviationPX = blobs[i].predictedPos.x - (*prev)[j].predictedPos.x;
            float deviationPY = blobs[i].predictedPos.y - (*prev)[j].predictedPos.y;
            float errorP = (float)sqrt( deviationPX*deviationPX
                                     + deviationPY*deviationPY );
	...
	if( best_error_ndx != -1 ) {
		for( i=0; i<cursize; i++ ) {
			...
			if( blobs[i].id != -1 ) {
				...
				// CALC DIRECTION AND PREDICTED POSITION
				...
				// CALC LIFESPAN
				blobs[i].lifeSpan = oldblob->lifeSpan+1;
